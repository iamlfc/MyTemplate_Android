package com.sum.main.ui.login.login
...
import com.sum.main.databinding.ActivityLoginBinding
import com.sum.main.ui.login.policy.PrivacyPolicyActivity
import com.sum.main.ui.login.register.RegisterActivity
    ...
    override fun initData() {
        mViewModel.loginLiveData.observe(this) { user ->
            ...
            user?.let {
                ...
                TipsToast.showTips(com.sum.common.R.string.success_login)
//                MainServiceProvider.toMain(context = this)
    ...
    private fun initListener() {
        ...
        mBinding.tvForgetPassword.onClick {
            TipsToast.showTips(com.sum.common.R.string.login_forget_password)
        }
...
//                .filter { it.isNotEmpty() }
    ...
    private fun toLogin() {
        ...
        if (userName.isNullOrEmpty() || userName.length < 11) {
            TipsToast.showTips(getStringFromResource(com.sum.common.R.string.error_phone_number))
            return
        }
        if (password.isNullOrEmpty()) {
            TipsToast.showTips(com.sum.common.R.string.error_input_password)
            return
        }
        if (!mBinding.cbAgreement.isChecked) {
            TipsToast.showTips(com.sum.common.R.string.tips_read_user_agreement)
            return
        }
        showLoading()
    ...
    private fun setPasswordHide() {
        ...
        if (isShowPassword) {
            mBinding.ivPasswordToggle.setImageResource(com.sum.common.R.mipmap.ic_psw_invisible)
            mBinding.etPassword.transformationMethod = HideReturnsTransformationMethod.getInstance()
        } else {
            mBinding.etPassword.transformationMethod = PasswordTransformationMethod.getInstance()
            mBinding.ivPasswordToggle.setImageResource(com.sum.common.R.mipmap.ic_psw_visible)
        }
    ...
    private fun initAgreement() {
        val agreement = getStringFromResource(com.sum.common.R.string.login_agreement)
        try {
            mBinding.cbAgreement.movementMethod = LinkMovementMethod.getInstance()
            val spaBuilder = SpannableStringBuilder(agreement)
            val privacySpan = getStringFromResource(com.sum.common.R.string.login_privacy_agreement)
            val serviceSpan = getStringFromResource(com.sum.common.R.string.login_user_agreement)
            spaBuilder.setSpan(
