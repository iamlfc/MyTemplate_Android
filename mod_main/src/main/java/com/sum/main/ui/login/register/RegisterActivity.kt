package com.sum.main.ui.login.register
...
import com.sum.main.databinding.ActivityRegisterBinding
...
class RegisterActivity : BaseMvvmActivity<ActivityRegisterBinding>() {
    ...
    private fun toRegister() {
        ...
        if (password.isNullOrEmpty() || repassword.isNullOrEmpty() || password != repassword) {
            TipsToast.showTips(com.sum.common.R.string.error_double_password)
            return
        }
        ...
        mViewModel.register(userName, password, repassword).observe(this) { user ->
            dismissLoading()
            user?.let {
                TipsToast.showTips(com.sum.common.R.string.success_register)
                //保存用户信息
